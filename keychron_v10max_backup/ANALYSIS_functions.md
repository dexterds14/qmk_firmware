...
* **leader_start_user** (0x0800CDF4): `rgb_matrix_set_color_all(255,0,255)`. **leader_end_user** (0x0800CDFE): `rgb_matrix_set_color_all(0,0,0)`, then `if/else if` chain: F -> LGUI+P, LGUI+K; (A,Q) -> LCTL+A, Z, Q, ENTER; Y -> LCTL+X, Y, ENTER; (LCTL,I) -> LCTL+LALT+I; J -> LGUI+P; F -> LGUI+QUOTE (dead: shadowed by the first F test; the sibling source has S/H/C/R entries there); DEL -> LGUI+UP, LEFT. All with register_code/unregister_code pairs (no tap_code16/SEND_STRING in the image).
...
* The second `leader_sequence_one_key(KC_F)` branch (LGUI+QUOTE) is unreachable; the sibling source has S/H/C/R sequences in that position - the 2024 firmware simply had two F entries.
